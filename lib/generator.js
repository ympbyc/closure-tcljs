...
function parseStatement(statement, declarations) {
  ...
  if (typedefTag) {
    if (typedefTag.type.type === "RecordType")
        moduleInfo.interfaces.push({
            name: name,
            members: getInterfaceMembersFromTypedef(typedefTag.type),
            comment: comment
        });
    if (typedefTag.type.type === "UnionType")
        moduleInfo.aliases.push({
            name: name,
            elements: getUnionElementsFromTypeDef(typedefTag.type),
            comment: comment
        });
    return;
  }
...
function getUnionElementsFromTypeDef (type) {
  if (type.type === 'UnionType')
    return type.elements.map(getTsType);
  return [];
}
...
function getTsType(type, opt_isChildOfTypeApplication) {
  ...
  switch (type.type) {
    ...
    case 'NonNullableType':
      ...
      return getTsType(type.expression);
    case 'UnionType':
      return '(U ' + type.elements.map(getTsType).join(' ') + ')';
    case 'RestType':
...
function toRecordTypeString(tag) {
  return '(HMap :mandatory {' + tag.fields.map(function(field) {
    return ':' + field.key + ' ' + getTsType(field.value);
  }).join(' ') + '})';
}
...
function toFunctionArgsString(params) {
  return '[' + params.map(function(param) {
    return param.type; //ignore param.name
  }).join(' ');
}
...
function outputModule(moduleDeclaration, name) {
  ...
  output.push(moduleDeclaration.aliases.map(outputAliasDeclaration.bind(null, indent)).join('\n'));
...
function outputVarDeclaration(indent, declare) {
  var output = []; //('/*' + declare.comment.value + '*/').split('\n');
  output.push('(ann ' + declare.name + ' ' + declare.type + ')');
  return '\n' + output.map(function(line) {return indent + line;}).join('\n');
}

function outputAliasDeclaration(indent, declare) {
  var output = [];
  output.push('(def-alias ' + declare.name + ' (U '); //todo non-unions
  declare.elements.forEach(function (element) {
    output.push('    ' + element + ' ');
  });
  output.push('))');
  return '\n' + output.map(function(line) {return indent + line;}).join('\n');
...
function outputFunctionDeclaration(indent, declare) {
  var output = []; //('/*' + declare.comment.value + '*/').split('\n');
  output.push('(ann ' + declare.name + ' (All ' +
              getTemplateString(declare.templates) + ' ' +
              declare.type + '))');
  return '\n' + output.map(function(line) {return indent + line;}).join('\n');
}
...
function outputInterfaceDeclaration(indent, declare) {
  var output = []; //('/*' + declare.comment.value + '*/').split('\n');
  output.push('(def-jsnominal ' + declare.name + ' ');
...
function outputClassDeclaration(indent, declare) {
  ...
  if (declare.type === 'ClassType') {
    //TODO extends
    output.push('(ann ' + declare.name + '. ' + declare.cstr + ')');
    output.push('(ann-datatype ' +
                declare.name + ' (All ' +
                getTemplateString(declare.templates) + ' [');
  } else if (declare.type === 'InterfaceType') {
    output.push('(def-jsnominal ' + declare.name + ' (All ' +
                getTemplateString(declare.templates) + ' [');
  }
  ...
  declare.methods.forEach(function(method) {
    ...
    output.push('    ' + method.name + ' :- (All ' +
                getTemplateString(method.templates) + ' ' +
                method.type + ')');
  });
  output.push(']))');
  return '\n' + output.map(function(line) {return indent + line;}).join('\n');
